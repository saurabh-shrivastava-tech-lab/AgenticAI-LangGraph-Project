...
def generate_topic(state:WorkflowState)->WorkflowState:
    """Node 1 ASK LLM to pick an ineresting kids learning topic """
    print("\n [Node 1] Generating topic....")

    response = llm.invoke("Suggest one short kids learning topic (just the topic name, no extra explanation).")
    topic = response.content.strip()
    print(f"    ->Topic:{topic}")
    return {"topic":topic}

def write_paragraph(state:WorkflowState)->WorkflowState:
    """Node 2 Write Short paragraph from Node 2 in one sentence. """
    print("\n [Node 2] Wrting paragraph.....")
    prompt = f"write a short 3- sentrncrs paragarph about :{state['topic']}"
...
#If I am the main program → run this code
#If I am being used as a library → don’t run it
